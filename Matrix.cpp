...
void Matrix::setRotate(const Quat& q)
{
	...
	if (fabs(length2) <= std::numeric_limits<float>::min())
	{
...
{
	const Matrix::value_type* lhs = reinterpret_cast<const Matrix::value_type*>(_mat);
	const Matrix::value_type* end_lhs = lhs + 16;
	const Matrix::value_type* rhs = reinterpret_cast<const Matrix::value_type*>(m._mat);
...
void Matrix::makeRotate(value_type angle, const Vec3& axis)
{
	makeIdentity();
...
void Matrix::makeRotate(value_type angle1, const Vec3& axis1,
...
bool Matrix::transpose(const Matrix &mat)
{
	if (&mat == this) {
		Matrix tm(mat);
		return transpose(tm);
...
void Matrix::orthoNormalize(const Matrix& rhs)
{
	...
	if (!equivalent((float)x_colMag, 1.0f) && !equivalent((float)x_colMag, 0.0f))
	{
	...
	if (!equivalent((float)y_colMag, 1.0f) && !equivalent((float)y_colMag, 0.0f))
	{
	...
	if (!equivalent((float)z_colMag, 1.0f) && !equivalent((float)z_colMag, 0.0f))
	{
...
bool Matrix::invert_4x4(const Matrix& mat)
{
	if (&mat == this) {
		Matrix tm(mat);
...
void Matrix::makeOrtho(float left, float right,
...
bool Matrix::getOrtho(Matrix::value_type& left, Matrix::value_type& right,
	Matrix::value_type& bottom, Matrix::value_type& top,
	Matrix::value_type& zNear, Matrix::value_type& zFar) const
{
...
void Matrix::makeFrustum(float left, float right,
	float bottom, float top,
	float zNear, float zFar)
{
	// note transpose of Matrix_implementation wr.t OpenGL documentation, since the OSG use post multiplication rather than pre.
	...
	float C = (fabs(zFar)>DBL_MAX) ? -1. : -(zFar + zNear) / (zFar - zNear);
	float D = (fabs(zFar)>DBL_MAX) ? -2.*zNear : -2.0*zFar*zNear / (zFar - zNear);
...
bool Matrix::getFrustum(Matrix::value_type& left, Matrix::value_type& right,
	Matrix::value_type& bottom, Matrix::value_type& top,
	Matrix::value_type& zNear, Matrix::value_type& zFar) const
{
	...
	Matrix::value_type temp_near = _mat[3][2] / (_mat[2][2] - 1.0);
	Matrix::value_type temp_far = _mat[3][2] / (1.0 + _mat[2][2]);
...
void Matrix::makePerspective(float fovy, float aspectRatio,
	float zNear, float zFar)
{
...
bool Matrix::getPerspective(Matrix::value_type& fovy, Matrix::value_type& aspectRatio,
	Matrix::value_type& zNear, Matrix::value_type& zFar) const
{
	Matrix::value_type right = 0.0;
	Matrix::value_type left = 0.0;
	Matrix::value_type top = 0.0;
	Matrix::value_type bottom = 0.0;

	// note: near and far must be used inside this method instead of zNear and zFar
	// because zNear and zFar are references and they may point to the same variable.
	Matrix::value_type temp_near = 0.0;
	Matrix::value_type temp_far = 0.0;
...
void Matrix::getLookAt(Vec3& eye, Vec3& center, Vec3& up, value_type lookDistance) const
{
	Matrix inv;
	inv.invert(*this);
	...
	Vec3 e = Vec3(0.0, 0.0, 0.0)*inv;
	up = transform3x3(*this, Vec3(0.0, 1.0, 0.0));
	Vec3 c = transform3x3(*this, Vec3(0.0, 0.0, -1));
	c.normalize();
	c = e + c * lookDistance;
