...
void Quat::makeRotate( const Vec3& from, const Vec3& to )
{
    ...
    float dotProdPlus1 = 1.0 + sourceVector * targetVector;
    ...
    if (dotProdPlus1 < 1e-7) {
        ...
        if (fabs(sourceVector.x()) < 0.6) {
            const float norm = sqrt(1.0 - sourceVector.x() * sourceVector.x());
            _v[0] = 0.0;
            _v[1] = sourceVector.z() / norm;
            _v[2] = -sourceVector.y() / norm;
            _v[3] = 0.0;
        } else if (fabs(sourceVector.y()) < 0.6) {
            const float norm = sqrt(1.0 - sourceVector.y() * sourceVector.y());
            _v[0] = -sourceVector.z() / norm;
            _v[1] = 0.0;
            _v[2] = sourceVector.x() / norm;
            _v[3] = 0.0;
        } else {
            const float norm = sqrt(1.0 - sourceVector.z() * sourceVector.z());
        ...
        const float s = sqrt(0.5 * dotProdPlus1);
        const Vec3 tmp = sourceVector ^ targetVector / (2.0*s);
        _v[0] = tmp.x();
        _v[1] = tmp.y();
        _v[2] = tmp.z();
        _v[3] = s;
...
void Quat::makeRotate_original( const Vec3& from, const Vec3& to )
{
    const value_type epsilon = 0.0000001;
    ...
    {
		cout << "*** Quat::makeRotate(from,to) with near co-linear vectors, epsilon= " << fabs(cosangle - 1) << std::endl;
