...
inline Vec4::value_type operator * (const Vec3& lhs,const Vec4& rhs)
{
    return lhs[0]*rhs[0]+lhs[1]*rhs[1]+lhs[2]*rhs[2]+rhs[3];
}

/** Compute the dot product of a Vec4f and a (Vec3,1.0). */
inline Vec4::value_type operator * (const Vec4& lhs,const Vec3& rhs)
{
    return lhs[0]*rhs[0]+lhs[1]*rhs[1]+lhs[2]*rhs[2]+lhs[3];
}

/** multiply by vector components. */
inline Vec4 componentMultiply(const Vec4& lhs, const Vec4& rhs)
{
    return Vec4(lhs[0]*rhs[0], lhs[1]*rhs[1], lhs[2]*rhs[2], lhs[3]*rhs[3]);
}

/** divide rhs components by rhs vector components. */
inline Vec4 componentDivide(const Vec4& lhs, const Vec4& rhs)
{
    return Vec4(lhs[0]/rhs[0], lhs[1]/rhs[1], lhs[2]/rhs[2], lhs[3]/rhs[3]);
}
